...
class AccessRequestMonthlyLimitTests(SecurityAwareAPITestCase):
    """A person (identified by email) may submit at most 4 access requests
    ...
    def _submit(self, email):
        return self.client.post(
            self.endpoint,
            {
                ...
                "description": "Need access for coursework",
            },
            format="json",
        )

    def test_allows_up_to_four_requests_per_month(self):
        for i in range(self.limit):
            response = self._submit("jane@pcampus.edu.np")
            self.assertEqual(response.status_code, status.HTTP_201_CREATED, response.data)

        self.assertEqual(
            AccessRequest.objects.filter(email="jane@pcampus.edu.np").count(),
            self.limit,
        )

    def test_rejects_fifth_request_same_month(self):
        for _ in range(self.limit):
            self._submit("jane@pcampus.edu.np")

        response = self._submit("jane@pcampus.edu.np")
        self.assertEqual(response.status_code, status.HTTP_429_TOO_MANY_REQUESTS)
        self.assertEqual(response.data["statusCode"], status.HTTP_429_TOO_MANY_REQUESTS)
        self.assertFalse(response.data["success"])

    def test_other_email_unaffected(self):
        for _ in range(self.limit):
            self._submit("jane@pcampus.edu.np")

        response = self._submit("john@pcampus.edu.np")
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)

    def test_rejects_non_campus_email(self):
        response = self._submit("jane@example.com")
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertFalse(response.data["success"])

    def test_campus_domain_is_case_insensitive(self):
        response = self._submit("JANE@PCAMPUS.EDU.NP")
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        self.assertEqual(
            response.data["data"]["email"], "jane@pcampus.edu.np"
        )

    def test_limit_resets_after_30_days(self):
        for _ in range(self.limit):
            req = AccessRequest.objects.create(
                fullName="Jane Doe",
                email="jane@pcampus.edu.np",
                department="Computer Science",
                description="Old request",
            )
            AccessRequest.objects.filter(pk=req.pk).update(
...
class BlockedRequesterTests(SecurityAwareAPITestCase):
    """Admins can blacklist an email so it can no longer submit access requests."""

    request_endpoint = "/api/v1/access-requests/"
    blocked_endpoint = "/api/v1/blocked-requesters/"
    ...
    def _login(self, username, password):
        ...
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        self.client.credentials(
            HTTP_AUTHORIZATION=f"Bearer {response.data['data']['accessToken']}"
        )
        return response

    def _block(self, email):
        return self.client.post(self.blocked_endpoint, {"email": email}, format="json")

    def test_unauthenticated_cannot_manage_blocks(self):
        response = self._block("spam@pcampus.edu.np")
        self.assertEqual(response.status_code, status.HTTP_401_UNAUTHORIZED)

    def test_non_admin_cannot_manage_blocks(self):
        ...
        self._login("inventory-user", "another-safe-password")
        response = self._block("spam@pcampus.edu.np")
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)

    def test_block_rejects_new_submissions(self):
        self._login("admin", self.admin_password)
        block = self._block("spam@pcampus.edu.np")
        self.assertEqual(block.status_code, status.HTTP_201_CREATED)
        self.client.credentials()
        response = self._submit("spam@pcampus.edu.np")
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
        self.assertEqual(response.data["statusCode"], status.HTTP_403_FORBIDDEN)

    def test_block_is_case_insensitive(self):
        self._login("admin", self.admin_password)
        self._block("Spam@PCampus.EDU.NP")
        self.client.credentials()
        response = self._submit("spam@pcampus.edu.np")
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
    ...
    def test_blocking_is_idempotent(self):
        self._login("admin", self.admin_password)
        first = self._block("spam@pcampus.edu.np")
        second = self._block("SPAM@pcampus.edu.np")
        self.assertEqual(first.status_code, status.HTTP_201_CREATED)
        self.assertEqual(second.status_code, status.HTTP_201_CREATED)
        self.assertEqual(
            first.data["data"]["_id"], second.data["data"]["_id"]
        )

    def test_list_blocked(self):
        self._login("admin", self.admin_password)
        self._block("spam@pcampus.edu.np")
        self._block("junk@pcampus.edu.np")
        listing = self.client.get(self.blocked_endpoint)
        self.assertEqual(listing.status_code, status.HTTP_200_OK)
        self.assertEqual(listing.data["data"]["totalBlocked"], 2)
        emails = {r["email"] for r in listing.data["data"]["blockedRequesters"]}
        self.assertEqual(emails, {"spam@pcampus.edu.np", "junk@pcampus.edu.np"})

    def test_unblock_restores_submission(self):
        self._login("admin", self.admin_password)
        blocked = self._block("spam@pcampus.edu.np")
        requester_id = blocked.data["data"]["_id"]
        unblock = self.client.delete(f"{self.blocked_endpoint}{requester_id}")
        self.assertEqual(unblock.status_code, status.HTTP_200_OK)
        self.client.credentials()
        response = self._submit("spam@pcampus.edu.np")
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)

    def test_block_missing_email_and_invalid_email_rejected(self):
        self._login("admin", self.admin_password)
        missing = self.client.post(self.blocked_endpoint, {}, format="json")
        self.assertEqual(missing.status_code, status.HTTP_400_BAD_REQUEST)
        invalid = self._block("not-an-email")
